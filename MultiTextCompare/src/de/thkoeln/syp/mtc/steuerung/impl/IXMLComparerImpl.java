...
public class IXMLComparerImpl {
	...
	public double compare(File ref, File comp) throws IOException, JDOMException{
		...
		for(Element currentRef : refFirstLevelChildren){
			...
			if(currentComp != null){
				...
				if(hasChildren(currentRef) && hasChildren(currentComp)){
					 compareElementsRecursively(currentRef, currentComp, currentLevelWeight);
				}
				// einzelnes Feld
				else if(!hasChildren(currentRef) && !hasChildren(currentComp)){
					similarity += compareElements(currentRef, currentComp, currentLevelWeight);
				}
			}
		}
		return similarity;
	...
	private double compareElements(Element ref, Element comp, double currentLevelWeight){
		...
		contentSimilarity = currentLevelWeight * contentWeight * compareStrings(ref.getValue(), comp.getValue());
		attributeSimilarity = currentLevelWeight * attributeWeight * calcAttributeSimilarity(ref.getAttributes(), comp.getAttributes());
		totalSimilarity = contentSimilarity + attributeSimilarity;		
		return totalSimilarity;
	}
	
	private double calcAttributeSimilarity(List<Attribute> refAttr, List<Attribute> compAttr){
		...
		for(int i = 0; i < matchingRef.size(); i++){
			similarity += (1.0/ maxSize) * compareStrings(matchingRef.get(i).getValue(), matchingComp.get(i).getValue());
		}
		
		return similarity;
	}
	
	private void compareElementsRecursively(Element ref, Element comp, double currentLevelWeight){
		
	}
	
	private double compareStrings(String ref, String comp){
		double maxLength = Math.max(ref.length(), comp.length());
		double similarity = 1 - (textvergleicher.calculateLevenshteinDist(ref, comp, maxLineLength) / maxLength);
		return similarity;
	}
